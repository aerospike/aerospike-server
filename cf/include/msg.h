...
void msg_type_register(msg_type type, const msg_template* mt, size_t mt_sz,
		size_t scratch_sz);
...
size_t msg_to_iov_buf(const msg* m, uint8_t* buf, size_t buf_sz,
		uint32_t* msg_sz_r);
...
bool msg_parse_hdr(uint32_t* size_r, msg_type* type_r, const uint8_t* buf,
		size_t sz);
...
void msg_set_buf(msg* m, int field_id, const uint8_t* v, size_t sz,
		msg_set_type type);
...
void msg_msgpack_list_set_uint32(msg* m, int field_id, const uint32_t* buf,
		uint32_t count);
void msg_msgpack_list_set_uint64(msg* m, int field_id, const uint64_t* buf,
		uint32_t count);
...
int msg_get_buf(const msg* m, int field_id, uint8_t** buf_r, size_t* sz_r,
		msg_get_type type);
cf_digest* msg_get_digest(const msg* m, int field_id);

int msg_get_uint32_array(const msg* m, int field_id, uint32_t idx,
		uint32_t* val_r);
int msg_get_uint64_array_count(const msg* m, int field_id, uint32_t* count_r);
int msg_get_uint64_array(const msg* m, int field_id, uint32_t idx,
		uint64_t* val_r);

bool msg_msgpack_list_get_count(const msg* m, int field_id, uint32_t* count_r);
bool msg_msgpack_list_get_uint32_array(const msg* m, int field_id,
		uint32_t* buf_r, uint32_t* count_r);
bool msg_msgpack_list_get_uint64_array(const msg* m, int field_id,
		uint64_t* buf_r, uint32_t* count_r);
bool msg_msgpack_list_get_buf_array(const msg* m, int field_id, cf_vector* v_r,
		bool init_vec);
