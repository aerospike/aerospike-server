...
typedef void (*cf_ip_addr_from_string_cb)(bool is_resolved, const char* name,
		const cf_ip_addr* addrs, const uint32_t n_addrs, void* udata);

void cf_socket_set_advertise_ipv6(bool advertise);
bool cf_socket_advertises_ipv6(void);

CF_MUST_CHECK int32_t cf_ip_addr_from_string(const char* string,
		cf_ip_addr* addr);
CF_MUST_CHECK int32_t cf_ip_addr_from_string_multi(const char* string,
		cf_ip_addr* addrs, uint32_t* n_addrs);
void cf_ip_addr_from_string_multi_a(const char* string,
		cf_ip_addr_from_string_cb cb, void* udata);
CF_MUST_CHECK int32_t cf_ip_addr_to_string(const cf_ip_addr* addr, char* string,
		size_t size);
void cf_ip_addr_to_string_safe(const cf_ip_addr* addr, char* string, size_t size);
CF_MUST_CHECK int32_t cf_ip_addr_to_string_multi(const cf_ip_addr* addrs,
		uint32_t n_addrs, char* string, size_t size);
CF_MUST_CHECK int32_t cf_ip_addr_from_binary(const uint8_t* binary, size_t size,
		cf_ip_addr* addr);
CF_MUST_CHECK int32_t cf_ip_addr_to_binary(const cf_ip_addr* addr,
		uint8_t* binary, size_t size);

CF_MUST_CHECK int32_t cf_ip_addr_compare(const cf_ip_addr* lhs,
		const cf_ip_addr* rhs);
...
CF_MUST_CHECK int32_t cf_ip_net_to_string(const cf_ip_net* net, char* string,
		size_t size);
CF_MUST_CHECK bool cf_ip_net_contains(const cf_ip_net* net,
		const cf_ip_addr* addr);

CF_MUST_CHECK int32_t cf_ip_port_from_string(const char* string,
		cf_ip_port* port);
CF_MUST_CHECK int32_t cf_ip_port_to_string(cf_ip_port port, char* string,
		size_t size);
void cf_ip_port_to_string_safe(cf_ip_port port, char* string, size_t size);
CF_MUST_CHECK int32_t cf_ip_port_from_binary(const uint8_t* binary, size_t size,
		cf_ip_port* port);
CF_MUST_CHECK int32_t cf_ip_port_to_binary(cf_ip_port port, uint8_t* binary,
		size_t size);

CF_MUST_CHECK int32_t cf_ip_addr_port_to_string(const cf_ip_addr* addr,
		cf_ip_port port, char* string, size_t size);
void cf_ip_addr_port_to_string_safe(const cf_ip_addr* addr, cf_ip_port port,
		char* string, size_t size);

CF_MUST_CHECK int32_t cf_sock_addr_to_string(const cf_sock_addr* addr,
		char* string, size_t size);
void cf_sock_addr_to_string_safe(const cf_sock_addr* addr, char* string,
		size_t size);

CF_MUST_CHECK int32_t cf_sock_addr_from_host_port(const char* host,
		cf_ip_port port, cf_sock_addr* addr);
void cf_sock_addr_from_addr_port(const cf_ip_addr* ip_addr, cf_ip_port port,
		cf_sock_addr* addr);

CF_MUST_CHECK int32_t cf_sock_addr_compare(const cf_sock_addr* lhs,
		const cf_sock_addr* rhs);
...
CF_MUST_CHECK int32_t cf_serv_cfg_add_sock_cfg(cf_serv_cfg* serv_cfg,
		const cf_sock_cfg* sock_cfg);

CF_MUST_CHECK bool cf_sockets_has_socket(const cf_sockets* socks,
		const cf_socket* sock);
...
void cf_socket_keep_alive(cf_socket* sock, uint32_t idle, uint32_t interval,
		uint32_t count);
...
static inline void
cf_socket_copy(const cf_socket* from, cf_socket* to)
{
	to->fd = from->fd;
	to->state = from->state;
	to->cfg = from->cfg;
...
void cf_socket_show_server(cf_log_context cont, const char* tag,
		const cf_sockets* socks);
CF_MUST_CHECK int32_t cf_socket_init_client(cf_sock_cfg* cfg, int32_t timeout,
		cf_socket* sock);

CF_MUST_CHECK int32_t cf_socket_accept(cf_socket* lsock, cf_socket* sock,
		cf_sock_addr* addr);
CF_MUST_CHECK int32_t cf_socket_remote_name(const cf_socket* sock,
		cf_sock_addr* addr);
CF_MUST_CHECK int32_t cf_socket_local_name(const cf_socket* sock,
		cf_sock_addr* addr);
CF_MUST_CHECK int32_t cf_socket_available(cf_socket* sock);

CF_MUST_CHECK int32_t cf_socket_recv_from(cf_socket* sock, void* buff,
		size_t size, int32_t flags, cf_sock_addr* addr);
CF_MUST_CHECK int32_t cf_socket_recv(cf_socket* sock, void* buff, size_t size,
		int32_t flags);
CF_MUST_CHECK int32_t cf_socket_recv_msg(cf_socket* sock, struct msghdr* m,
		int32_t flags);
CF_MUST_CHECK int32_t cf_socket_send_to(cf_socket* sock, const void* buff,
		size_t size, int32_t flags, const cf_sock_addr* addr);
CF_MUST_CHECK int32_t cf_socket_send(cf_socket* sock, const void* buff,
		size_t size, int32_t flags);
CF_MUST_CHECK int32_t cf_socket_send_msg(cf_socket* sock, struct msghdr* m,
		int32_t flags);

CF_MUST_CHECK int32_t cf_socket_recv_all(cf_socket* sock, void* buff,
		size_t size, int32_t flags, int32_t timeout);
CF_MUST_CHECK int32_t cf_socket_send_all(cf_socket* sock, const void* buff,
		size_t size, int32_t flags, int32_t timeout);
CF_MUST_CHECK int32_t cf_socket_try_send_all(cf_socket* sock, const void* buff,
		size_t size, int32_t flags);
...
CF_MUST_CHECK int32_t cf_mserv_cfg_add_msock_cfg(cf_mserv_cfg* serv_cfg,
		const cf_msock_cfg* sock_cfg);
CF_MUST_CHECK int32_t cf_mserv_cfg_add_combo(cf_mserv_cfg* serv_cfg,
		cf_sock_owner owner, cf_ip_port port, cf_ip_addr* addr,
		cf_ip_addr* if_addr, uint8_t ttl);

CF_MUST_CHECK int32_t cf_socket_mcast_init(cf_mserv_cfg* cfg, cf_sockets* socks);
void cf_socket_mcast_show(cf_log_context cont, const char* tag,
		const cf_sockets* socks);
CF_MUST_CHECK int32_t cf_socket_mcast_set_inter(cf_socket* sock,
		const cf_ip_addr* iaddr);
CF_MUST_CHECK int32_t cf_socket_mcast_set_ttl(cf_socket* sock, int32_t ttl);
CF_MUST_CHECK int32_t cf_socket_mcast_join_group(cf_socket* sock,
		const cf_ip_addr* iaddr, const cf_ip_addr* gaddr);

void cf_poll_create(cf_poll* poll);

static inline bool
cf_poll_equal(const cf_poll poll1, const cf_poll poll2)
{
	return CEFD(poll1) == CEFD(poll2);
}

void cf_poll_add_fd(cf_poll poll, int32_t fd, uint32_t events, void* data);
void cf_poll_add_socket(cf_poll poll, const cf_socket* sock, uint32_t events,
		void* data);
CF_MUST_CHECK int32_t cf_poll_modify_socket_forgiving(cf_poll poll,
		const cf_socket* sock, uint32_t events, void* data, uint32_t n_err_ok,
		const int32_t* err_ok);
CF_MUST_CHECK int32_t cf_poll_delete_fd_forgiving(cf_poll poll, int32_t fd,
...
CF_MUST_CHECK int32_t cf_poll_wait(cf_poll poll, cf_poll_event* events,
		int32_t limit, int32_t timeout);
void cf_poll_destroy(cf_poll poll);

static inline void
cf_poll_modify_socket(cf_poll poll, const cf_socket* sock, uint32_t events,
		void* data)
{
	CF_IGNORE_ERROR(cf_poll_modify_socket_forgiving(poll, sock, events, data, 0,
			NULL));
}

static inline CF_MUST_CHECK int32_t
cf_poll_delete_socket_forgiving(cf_poll poll, const cf_socket* sock,
		uint32_t n_err_ok, int32_t* err_ok)
{
...
static inline void
cf_poll_delete_fd(cf_poll poll, int32_t fd)
{
	CF_IGNORE_ERROR(cf_poll_delete_fd_forgiving(poll, fd, 0, NULL));
}

static inline void
cf_poll_delete_socket(cf_poll poll, const cf_socket* sock)
{
...
CF_MUST_CHECK int32_t cf_inter_addr_to_index_and_name(const cf_ip_addr* addr,
		int32_t* index, char** name);
CF_MUST_CHECK int32_t cf_inter_get_physical(const char* if_name,
		char* phys_name, uint32_t phys_name_sz);
...
CF_MUST_CHECK int32_t cf_node_id_get(cf_ip_port port, const char* if_hint,
		cf_node* id);

#if defined CF_SOCKET_PRIVATE
CF_MUST_CHECK size_t cf_socket_addr_len(const struct sockaddr* sa);
CF_MUST_CHECK int32_t cf_socket_parse_netlink(bool allow_v6, uint32_t family,
		uint32_t flags, const void* data, size_t len, cf_ip_addr* addr);
...
CF_MUST_CHECK int32_t cf_ip_addr_from_addrinfo(const char* name,
		const addrinfo* info, cf_ip_addr* addrs, uint32_t* n_addrs);
#endif
