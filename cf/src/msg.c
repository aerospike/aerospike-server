...
static uint32_t msg_field_write_hdr(const msg_field* mf, msg_field_type type,
		uint8_t* buf);
static uint32_t msg_field_write_buf(const msg_field* mf, msg_field_type type,
		uint8_t* buf);
static void msg_field_save(msg* m, msg_field* mf);
static bool msgpack_list_unpack_hdr(msgpack_in* mp, const msg* m, int field_id,
		uint32_t* count_r);
...
{
	cf_assert(type < M_TYPE_MAX && g_mte[type].mt != NULL, CF_MSG,
			"invalid type %u", type);
...
msg_to_iov_buf(const msg* m, uint8_t* buf, size_t buf_sz, uint32_t* msg_sz_r)
{
	...
	cf_assert(buf_sz >= max_iov * sizeof(struct iovec) + sizeof(msg_hdr) +
							int_fields *
									(sizeof(msg_field_hdr) + sizeof(uint64_t)),
	...
	cf_assert(ptr <= buf + buf_sz, AS_PARTICLE,
			"ptr out of bounds %p > buf %p + buf_sz %zu", ptr, buf, buf_sz);
	*msg_sz_r = body_sz + sizeof(msg_hdr);
...
msg_parse(msg* m, const uint8_t* buf, size_t bufsz)
{
	...
	if (bufsz < sz + sizeof(msg_hdr)) {
		cf_warning(CF_MSG, "msg_parse() bufsz %zu < msg sz %u + %zu", bufsz, sz,
				sizeof(msg_hdr));
		return false;
	}

	if (m->type != type) {
		cf_ticker_warning(CF_MSG, "parsed type %d for msg type %d", type,
				m->type);
		return false;
	}
...
msg_parse_fields(msg* m, const uint8_t* buf, size_t sz)
{
	...
	while (left != 0) {
		...
		if (mf && ft != mf_type(mf, m->type)) {
			cf_ticker_warning(CF_MSG,
					"msg type %d: parsed type %d for field type %d", m->type,
					ft, mf_type(mf, m->type));
			mf = NULL;
		}
		...
		if (mf) {
			...
			case M_FT_MSGPACK:
				...
				break;
			default:
				cf_ticker_detail(CF_MSG,
						"msg_parse: field type %d not supported - skipping",
						mf_type(mf, m->type));
				mf->is_set = false;
				break;
			}
...
void
msg_set_buf(msg* m, int field_id, const uint8_t* v, size_t sz, msg_set_type type)
{
	msg_field* mf = &m->f[field_id];
...
msg_set_uint32_array_size(msg* m, int field_id, uint32_t count)
{
	...
	cf_assert(! mf->is_set, CF_MSG,
			"msg_set_uint32_array_size() field already set");
...
msg_set_uint32_array(msg* m, int field_id, uint32_t idx, uint32_t v)
{
	...
	cf_assert(idx < (mf->field_sz >> 2), CF_MSG,
			"msg_set_uint32_array() idx out of bounds");
...
msg_set_uint64_array_size(msg* m, int field_id, uint32_t count)
{
	...
	cf_assert(! mf->is_set, CF_MSG,
			"msg_set_uint64_array_size() field already set");
...
msg_set_uint64_array(msg* m, int field_id, uint32_t idx, uint64_t v)
{
	...
	cf_assert(idx < (mf->field_sz >> 3), CF_MSG,
			"msg_set_uint64_array() idx out of bounds");
...
{
	cf_assert(field_id >= 0 && field_id < (int)m->n_fields, CF_MSG,
			"invalid field_id %d", field_id);
		...
		uint32_t* count_r)
{
	...
	for (uint32_t i = 0; i < count; i++) {
		...
		if (! msgpack_get_uint64(&mp, &val) || (val & (0xFFFFffffUL << 32)) != 0) {
			cf_warning(CF_MSG, "i %u/%u invalid packed uint32 val 0x%lx", i,
					count, val);
			return false;
		}
		...
		uint32_t* count_r)
{
	...
	for (uint32_t i = 0; i < count; i++) {
		...
		if (! msgpack_get_uint64(&mp, &val)) {
			cf_warning(CF_MSG, "i %u/%u invalid packed uint64 val 0x%lx", i,
					count, val);
			return false;
		}
		...
		bool init_vec)
{
	...
	if (init_vec) {
		...
		cf_assert(count <= cf_vector_capacity(v_r), CF_MSG,
				"count %u > vector cap %u", count, cf_vector_capacity(v_r));
	}
...
{
	cf_info(CF_MSG,
			"msg_dump: %s: msg %p rc %u n-fields %u bytes-used %u bytes-alloc'd %u type %d",
	...
	for (uint32_t i = 0; i < m->n_fields; i++) {
		...
		if (mf->is_set) {
			...
			case M_FT_ARRAY_UINT32:
				cf_info(CF_MSG,
						"   type ARRAY_UINT32: count %u n-uint32 %u free %c",
						mf->field_sz, mf->field_sz >> 2, mf->is_free ? 't' : 'f');
				{
					uint32_t n_ints = mf->field_sz >> 2;
					for (uint32_t j = 0; j < n_ints; j++) {
						cf_info(CF_MSG, "      idx %u value %u", j,
								cf_swap_from_be32(mf->u.ui32_a[j]));
					}
				}
				break;
			case M_FT_ARRAY_UINT64:
				cf_info(CF_MSG,
						"   type ARRAY_UINT64: count %u n-uint64 %u free %c",
						mf->field_sz, mf->field_sz >> 3, mf->is_free ? 't' : 'f');
				{
					uint32_t n_ints = mf->field_sz >> 3;
					for (uint32_t j = 0; j < n_ints; j++) {
						cf_info(CF_MSG, "      idx %u value %lu", j,
								cf_swap_from_be64(mf->u.ui64_a[j]));
					}
				}
				break;
			default:
