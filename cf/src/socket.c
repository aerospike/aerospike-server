...
CF_MUST_CHECK static int32_t cf_inter_get_addr_name(cf_ip_addr* addrs,
		uint32_t* n_addrs, const char* if_name);
...
int32_t
cf_ip_addr_to_string_multi(const cf_ip_addr* addrs, uint32_t n_addrs,
...
int32_t
cf_ip_addr_port_to_string(const cf_ip_addr* addr, cf_ip_port port, char* string,
		size_t size)
{
...
void
cf_ip_addr_port_to_string_safe(const cf_ip_addr* addr, cf_ip_port port,
		char* string, size_t size)
{
...
void
cf_sock_addr_from_addr_port(const cf_ip_addr* ip_addr, cf_ip_port port,
		cf_sock_addr* addr)
{
...
cf_serv_cfg_add_sock_cfg(cf_serv_cfg* serv_cfg, const cf_sock_cfg* sock_cfg)
{
	if (serv_cfg->n_cfgs >= CF_SOCK_CFG_MAX) {
		cf_warning(CF_SOCKET,
				"Too many socket configurations in server configuration");
		return -1;
	}
...
{
	return socks != NULL && sock >= &socks->socks[0] &&
			sock < &socks->socks[socks->n_socks];
}
...
safe_fcntl(int32_t fd, int32_t cmd, int32_t arg)
{
	...
	if (res < 0) {
		cf_crash(CF_SOCKET, "fcntl(%d) failed on FD %d: %d (%s)", cmd, fd,
...
safe_ioctl(int32_t fd, int32_t req, int32_t* arg)
{
	...
	if (res < 0) {
		cf_crash(CF_SOCKET, "ioctl(%d) failed on FD %d: %d (%s)", req, fd,
				errno, cf_strerror(errno));
	}
...
static void
safe_setsockopt(int32_t fd, int32_t level, int32_t name, const void* val,
		socklen_t len)
{
...
static void
forgive_setsockopt(int32_t fd, int32_t level, int32_t name, const void* val,
		socklen_t len)
{
	if (setsockopt(fd, level, name, val, len) < 0) {
...
{
	if (close(fd) < 0) {
		cf_crash(CF_SOCKET, "Error while closing FD %d: %d (%s)", fd, errno,
				cf_strerror(errno));
	}
}
		...
		uint32_t count)
{
	...
	if (interval > 0) {
		forgive_setsockopt(sock->fd, SOL_TCP, TCP_KEEPINTVL, &interval,
				sizeof(interval));
	}
...
cf_socket_init_server(cf_serv_cfg* cfg, cf_sockets* socks)
{
	...
	for (n = 0; n < cfg->n_cfgs; ++n) {
		...
		cf_debug(CF_SOCKET, "Initializing server for %s",
		...
		while (bind(sock->fd, (struct sockaddr*)&sas,
			...
			cf_warning(CF_SOCKET, "Socket %s in use, waiting",
					cf_sock_addr_print(&addr));
			usleep(5 * 1000 * 1000);
		}
...
void
cf_socket_show_server(cf_log_context cont, const char* tag,
		const cf_sockets* socks)
{
	for (uint32_t i = 0; i < socks->n_socks; ++i) {
...
connect_socket(const cf_socket* sock, struct sockaddr* sa, int32_t timeout)
{
	...
	if (errno != EINPROGRESS) {
		cf_ticker_warning(CF_SOCKET, "Error while connecting: %d (%s)", errno,
				cf_strerror(errno));
		goto cleanup0;
	}
	...
	if (efd < 0) {
		cf_ticker_warning(CF_SOCKET, "epoll_create() failed: %d (%s)", errno,
				cf_strerror(errno));
		goto cleanup0;
	}
	...
	if (epoll_ctl(efd, EPOLL_CTL_ADD, sock->fd, &event) < 0) {
		cf_crash(CF_SOCKET, "epoll_ctl() failed for FD %d: %d (%s)", sock->fd,
				errno, cf_strerror(errno));
	}
	...
	if (err != 0) {
		cf_ticker_warning(CF_SOCKET, "Error while connecting: %d (%s)", err,
				cf_strerror(err));
		goto cleanup1;
	}
...
x_name(name_func func, const char* which, int32_t fd, cf_sock_addr* addr)
{
	...
	if (func(fd, (struct sockaddr*)&sas, &sas_len) < 0) {
		cf_warning(CF_SOCKET, "Error while getting %s name: %d (%s)", which,
				errno, cf_strerror(errno));
		return -1;
	}
...
int32_t
cf_socket_send_to(cf_socket* sock, const void* buff, size_t size, int32_t flags,
		const cf_sock_addr* addr)
{
	cf_assert(sock->ssl == NULL, CF_SOCKET,
			"cannot use cf_socket_send_to() with TLS");
	...
	if (res < 0) {
		cf_debug(CF_SOCKET, "Error while sending on FD %d: %d (%s)", sock->fd,
				errno, cf_strerror(errno));
	}
...
int32_t
cf_socket_recv_from(cf_socket* sock, void* buff, size_t size, int32_t flags,
		cf_sock_addr* addr)
{
	cf_assert(sock->ssl == NULL, CF_SOCKET,
			"cannot use cf_socket_recv_from() with TLS");
	...
	if (res < 0) {
		cf_debug(CF_SOCKET, "Error while receiving on FD %d: %d (%s)", sock->fd,
				errno, cf_strerror(errno));
	}
	else if (addr != NULL) {
		cf_sock_addr_from_native(sa, addr);
...
socket_wait(const cf_socket* sock, uint16_t events, int32_t timeout)
{
	...
	while (true) {
		...
		if (count < 0) {
			...
			cf_crash(CF_SOCKET, "Error while polling FD %d: %d (%s)", pfd.fd,
					errno, cf_strerror(errno));
		}

		if (count > 1) {
			cf_crash(CF_SOCKET, "Unexpected number of events on FD %d: %d",
					sock->fd, count);
		}
		...
		if (count < 0) {
			if (errno == EAGAIN) {
				...
				if (timeout != 0) {
					...
					if (events != 0) {
						cf_detail(CF_SOCKET, "Unexpected events 0x%x on FD %d",
								events, sock->fd);
						return off;
					}
				}

				cf_debug(CF_SOCKET, "Timeout during blocking send on FD %d",
						sock->fd);
				return off;
			}
...
int32_t
cf_socket_send_all(cf_socket* sock, const void* buff, size_t size,
...
int32_t
cf_socket_try_send_all(cf_socket* sock, const void* buff, size_t size,
		int32_t flags)
{
	if (sock->ssl) {
		...
		if (count < 0) {
			if (errno == EAGAIN) {
				...
				if (timeout != 0) {
					...
					if (events != 0) {
						cf_warning(CF_SOCKET, "Unexpected events 0x%x on FD %d",
								events, sock->fd);
						return off;
					}
				}

				cf_debug(CF_SOCKET, "Timeout during blocking receive on FD %d",
						sock->fd);
				errno = ETIMEDOUT;
				return -1;
			}
...
int32_t
cf_socket_recv_all(cf_socket* sock, void* buff, size_t size, int32_t flags,
		int32_t timeout)
{
	if (sock->ssl) {
...
x_shutdown(cf_socket* sock, int32_t how)
{
	...
	if (shutdown(sock->fd, how) < 0) {
		if (errno != ENOTCONN) {
			cf_crash(CF_SOCKET, "shutdown() failed on FD %d: %d (%s)", sock->fd,
					errno, cf_strerror(errno));
		}
		else {
...
cf_socket_mcast_init(cf_mserv_cfg* cfg, cf_sockets* socks)
{
	...
	for (n = 0; n < cfg->n_cfgs; ++n) {
		...
		cf_debug(CF_SOCKET, "Initializing multicast socket for %s",
				cf_sock_addr_print(&addr));
		...
		cf_info(CF_SOCKET, "Joining multicast group: %s",
				cf_ip_addr_print(&addr.addr));

		if (cf_socket_mcast_join_group(sock, &cfg->cfgs[n].if_addr, &addr.addr) <
				0) {
...
{
	cf_debug(CF_SOCKET,
			"Adding FD %d to epoll instance with FD %d, events = 0x%x", fd,
			poll.fd, events);
	struct epoll_event ev = { .events = events, .data.ptr = data };

	if (epoll_ctl(poll.fd, EPOLL_CTL_ADD, fd, &ev) < 0) {
		cf_crash(CF_SOCKET,
				"Error while adding FD %d to epoll instance %d: %d (%s)", fd,
				poll.fd, errno, cf_strerror(errno));
	}
}

void
cf_poll_add_socket(cf_poll poll, const cf_socket* sock, uint32_t events,
		void* data)
{
...
int32_t
cf_poll_modify_socket_forgiving(cf_poll poll, const cf_socket* sock,
		uint32_t events, void* data, uint32_t n_err_ok, const int32_t* err_ok)
{
#if defined VERY_CHATTY
	cf_detail(CF_SOCKET,
			"Modifying FD %d in epoll instance with FD %d, events = 0x%x",
	...
	if (epoll_ctl(poll.fd, EPOLL_CTL_MOD, sock->fd, &ev) < 0) {
		...
		cf_crash(CF_SOCKET,
				"Error while modifying FD %d in epoll instance %d: %d (%s)",
...
cf_poll_delete_fd_forgiving(cf_poll poll, int32_t fd, uint32_t n_err_ok,
		int32_t* err_ok)
{
	cf_detail(CF_SOCKET, "Deleting FD %d from epoll instance with FD %d", fd,
			poll.fd);
	...
	if (epoll_ctl(poll.fd, EPOLL_CTL_DEL, fd, NULL) < 0) {
		...
		cf_crash(CF_SOCKET,
				"Error while deleting FD %d from epoll instance %d: %d (%s)",
...
cf_poll_wait(cf_poll poll, cf_poll_event* events, int32_t limit, int32_t timeout)
{
	...
	while (true) {
		int32_t res =
				epoll_wait(poll.fd, (struct epoll_event*)events, limit, timeout);

		if (res >= 0) {
#if defined VERY_CHATTY
			if (cf_fault_filter[CF_SOCKET] >= CF_DETAIL) {
				cf_detail(CF_SOCKET,
						"Epoll instance with FD %d reports %d event(s)",
						poll.fd, res);

				for (int32_t i = 0; i < res; ++i) {
					cf_detail(CF_SOCKET, "Event #%d: 0x%x, %p", i,
		...
		if (errno != EINTR) {
			cf_crash(CF_SOCKET,
					"Error while waiting for events on epoll instance %d: %d (%s)",
					poll.fd, errno, cf_strerror(errno));
		}
	}
}
...
typedef void (*data_cb)(cb_context* cont, void* info, int32_t type, void* data,
		size_t len);
typedef void (*post_cb)(cb_context* cont);

static int32_t
netlink_dump(int32_t type, int32_t filter1, int32_t filter2a, int32_t filter2b,
		int32_t filter2c, int32_t filter2d, size_t size, reset_cb reset_fn,
		data_cb data_fn, post_cb post_fn, cb_context* cont)
{
	int32_t res = -1;
	...
	while (! done) {
		...
		if (len < 0) {
			cf_warning(CF_SOCKET,
					"Error while receiving netlink response: %d (%s)", errno,
					cf_strerror(errno));
			goto cleanup2;
		}
...
link_fn(cb_context* cont, void* info_, int32_t type, void* data, size_t len)
{
	...
	if (entry == NULL) {
		...
		entry->up = (info->ifi_flags & (IFF_UP | IFF_RUNNING)) ==
				(IFF_UP | IFF_RUNNING);
	}
	...
	if (type == IFLA_IFNAME) {
		...
		cf_detail(CF_SOCKET, "Collected interface MTU %s -> %u", entry->name,
				entry->mtu);
	}
...
addr_fn(cb_context* cont, void* info_, int32_t type, void* data, size_t len)
{
	...
	if (type == IFA_LABEL) {
		...
		if (cf_socket_parse_netlink(cont->allow_v6, info->ifa_family,
					info->ifa_flags, data, len, &cont->curr_address) < 0) {
			return;
		}
...
addr_fix_fn(cb_context* cont)
{
	...
	if (cont->has_label) {
		...
		if (by_label == NULL) {
			...
			memcpy(&by_label->mac_addr, &by_index->mac_addr,
					sizeof(by_label->mac_addr));
	...
	if (i >= MAX_ADDRS) {
		cf_warning(CF_SOCKET,
				"too many addresses for interface %s - truncating %s",
	...
	++entry->n_addrs;
	cf_detail(CF_SOCKET, "Collected interface address %s -> %s", entry->name,
			cf_ip_addr_print(addr));
}
...
route_fn(cb_context* cont, void* info_, int32_t type, void* data, size_t len)
{
	...
	if (type == RTA_DST) {
		if (cf_socket_parse_netlink(cont->allow_v6, info->rtm_family, 0, data,
					len, &cont->curr_address) < 0) {
...
enumerate_inter(inter_info* inter, bool allow_v6)
{
	...
	if (netlink_dump(RTM_GETLINK, RTM_NEWLINK, IFLA_IFNAME, IFLA_ADDRESS,
				IFLA_MTU, IFLA_MASTER, sizeof(struct ifinfomsg), NULL, link_fn,
				NULL, &cont) < 0) {
		cf_crash(CF_SOCKET, "Error while enumerating network links");
	}

	if (netlink_dump(RTM_GETADDR, RTM_NEWADDR, IFA_LABEL, IFA_ADDRESS,
				IFA_LOCAL, -1, sizeof(struct ifaddrmsg), reset_fn, addr_fn,
				addr_fix_fn, &cont) < 0) {
	...
	if (netlink_dump(RTM_GETROUTE, RTM_NEWROUTE, RTA_DST, RTA_OIF, RTA_PRIORITY,
				-1, sizeof(struct rtmsg), reset_fn, route_fn, route_fix_fn,
				&cont) < 0) {
	...
	if (cf_log_check_level(CF_SOCKET, CF_DETAIL)) {
		...
		for (uint32_t i = 0; i < inter->n_inters; ++i) {
			...
			cf_detail(CF_SOCKET, "Master = %s",
					entry->master.entry != NULL ? entry->master.entry->name
												: "(none)");
...
inter_get_addr(cf_ip_addr* addrs, uint32_t* n_addrs, inter_filter* filter)
{
	...
	for (uint32_t i = 0; i < inter.n_inters; ++i) {
		...
		for (uint32_t k = 0; k < entry->n_addrs; ++k) {
			...
			if (count >= *n_addrs) {
				cf_warning(CF_SOCKET,
						"Buffer overflow while enumerating interface addresses");
				return -1;
			}
...
int32_t
cf_inter_addr_to_index_and_name(const cf_ip_addr* addr, int32_t* index,
		char** name)
{
	inter_info inter;
	memset(&inter, 0, sizeof(inter));
...
int32_t
cf_inter_get_physical(const char* if_name, char* phys_name, uint32_t phys_name_sz)
{
	inter_info inter;
	memset(&inter, 0, sizeof(inter));
	enumerate_inter(&inter, true);
...
cf_inter_expand_bond(const char* if_name, char** out_names, uint32_t* n_out)
{
	...
	for (uint32_t i = 0; i < inter.n_inters; ++i) {
		...
		if (entry->master.entry == NULL ||
				strcmp(entry->master.entry->name, if_name) != 0) {
			continue;
		}
...
static const char* if_in_order[] = { "eth", "bond", "wlan", NULL };

static const char* if_default[] = { "^eth[[:digit:]]+$", "^bond[[:digit:]]+$",
	"^wlan[[:digit:]]+$", "^em[[:digit:]]+_[[:digit:]]+$",
	"^p[[:digit:]]+p[[:digit:]]+_[[:digit:]]+$", NULL };

static const char* if_default2[] = { "^em[[:digit:]]+$",
	"^p[[:digit:]]+p[[:digit:]]+$", NULL };

static const char* if_any[] = { "^.*$", NULL };
...
validate_inter(inter_entry* entry)
{
	...
	if (entry->mac_addr_len < 6) {
		cf_debug(CF_SOCKET, "Invalid MAC address length: %d",
				entry->mac_addr_len);
		return false;
	}
	...
	if (memcmp(entry->mac_addr, all0, 6) == 0 ||
			memcmp(entry->mac_addr, all1, 6) == 0) {
	...
	for (uint32_t i = 0; i < inter->n_inters; ++i) {
		...
		for (uint32_t k = 0; patterns[k] != NULL; ++k) {
			...
			if (regcomp(&rex, patterns[k], REG_EXTENDED | REG_NOSUB) != 0) {
				cf_crash(CF_SOCKET, "Error while compiling regular expression %s",
						patterns[k]);
			}

			bool ok = regexec(&rex, entry->name, 0, NULL, 0) == 0 &&
					validate_inter(entry);
			regfree(&rex);
...
cf_node_id_get(cf_ip_port port, const char* if_hint, cf_node* id)
{
	...
	if (if_hint != NULL) {
		...
		cf_warning(CF_SOCKET,
				"Unable to find interface %s specified in configuration file",
				if_hint);
		return -1;
	}
