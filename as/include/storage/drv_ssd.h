...
void ssd_cold_start_fill_orig(drv_ssd* ssd, const struct as_flat_record_s* flat,
		uint64_t rblock_id, const struct as_flat_opt_meta_s* opt_meta,
		struct as_index_tree_s* tree, struct as_index_ref_s* r_ref);
void ssd_cold_start_record_create(struct as_namespace_s* ns,
...
void ssd_cold_start_record_update(drv_ssds* ssds,
		const struct as_flat_record_s* flat,
		const struct as_flat_opt_meta_s* opt_meta, struct as_index_tree_s* tree,
		struct as_index_ref_s* r_ref);
void ssd_cold_start_adjust_cenotaph(struct as_namespace_s* ns,
...
void ssd_decrypt_whole(drv_ssd* ssd, uint64_t off, uint32_t n_rblocks,
...
void ssd_cold_start_init_repl_state(struct as_namespace_s* ns,
		struct as_index_s* r);
void ssd_cold_start_set_unrepl_stat(struct as_namespace_s* ns);

// XDR.
void ssd_cold_start_init_xdr_state(const struct as_flat_record_s* flat,
		struct as_index_s* r);
...
void ssd_header_init_cfg(const struct as_namespace_s* ns, drv_ssd* ssd,
		drv_header* header);
void ssd_header_validate_cfg(const struct as_namespace_s* ns, drv_ssd* ssd,
		drv_header* header);
...
BYTES_DOWN_TO_IO_MIN(drv_ssd* ssd, uint64_t bytes)
{
...
static inline uint64_t
BYTES_UP_TO_IO_MIN(drv_ssd* ssd, uint64_t bytes)
{
