...
#define define_rollback_alloc(_name, _alloc_buf, _rollback_size)               \
	uint8_t _name##_mem[sizeof(rollback_alloc) +                               \
			sizeof(void*) * (_alloc_buf ? 0 : _rollback_size)];                \
	rollback_alloc* const _name = (rollback_alloc*)_name##_mem;                \
	_name->ll_buf = _alloc_buf;                                                \
	_name->malloc_list_sz = 0;                                                 \
	_name->malloc_list_cap = (_alloc_buf ? 0 : _rollback_size)

#define DEFER_ROLLBACK_ALLOC(_name)                                            \
	DEFER_ATTR(_defer_rollback_alloc_internal)                                 \
	rollback_alloc* const DEFER_GLUE(_defer_rollback_alloc_, __LINE__) = (_name)
...
#define define_offset_index(_name, _contents, _content_sz, _ele_count)         \
	offset_index _name;                                                        \
	offset_index_init(&_name, NULL, _ele_count, _contents, _content_sz);       \
	uint8_t _name##_vlatemp[1 + offset_index_vla_sz(&_name)];                  \
	DEFER_ATTR(cdt_idx_defer_renull_free_fn)                                   \
	cdt_idx_defer_t _d##_name = { &_name };                                    \
	offset_index_alloc_temp(&_name, _name##_vlatemp, &_d##_name)

// NULL if !_cond
#define definep_cond_order_index2(_name, _max_idx, _alloc_count, _cond)               \
	uint8_t _name##_vlatemp[(_cond) ? sizeof(order_index) +                           \
							cdt_vla_sz(order_index_calc_size(_max_idx, _alloc_count)) \
									: 1];                                             \
	cdt_vla_ordidx_cast* _name##_cast = (cdt_vla_ordidx_cast*)_name##_vlatemp;        \
	order_index* _name = NULL;                                                        \
	DEFER_ATTR(cdt_idx_defer_renull_free_fn)                                          \
	cdt_idx_defer_t _d##_name = { .ordidx = _cond ? &_name##_cast->ordidx             \
												  : NULL };                           \
	if (_cond) {                                                                      \
		_name = &_name##_cast->ordidx;                                                \
		order_index_init2_temp(_name, _name##_vlatemp + sizeof(order_index),          \
				&_d##_name, _max_idx, _alloc_count);                                  \
	}

#define define_order_index(_name, _ele_count)                                  \
	order_index _name;                                                         \
	uint8_t _name##_vlatemp[1 +                                                \
			cdt_vla_sz(order_index_calc_size(_ele_count, _ele_count))];        \
	DEFER_ATTR(cdt_idx_defer_renull_free_fn)                                   \
	cdt_idx_defer_t _d##_name = { .ordidx = &_name };                          \
	order_index_init2_temp(&_name, _name##_vlatemp, &_d##_name, _ele_count,    \
			_ele_count)

#define define_order_index2(_name, _max_idx, _alloc_count)                     \
	order_index _name;                                                         \
	uint8_t _name##_vlatemp[1 +                                                \
			cdt_vla_sz(order_index_calc_size(_max_idx, _alloc_count))];        \
	DEFER_ATTR(cdt_idx_defer_renull_free_fn)                                   \
	cdt_idx_defer_t _d##_name = { .ordidx = &_name };                          \
	order_index_init2_temp(&_name, _name##_vlatemp, &_d##_name, _max_idx,      \
			_alloc_count)

#define define_int_list_builder(_name, _alloc, _count)                         \
	cdt_container_builder _name;                                               \
	cdt_int_list_builder_start(&_name, _alloc, _count)

#define define_list_builder(_name, _alloc, _ele_count, _max_sz)                \
	cdt_container_builder _name;                                               \
	cdt_list_builder_start(&_name, _alloc, _ele_count, _max_sz)

#define define_cdt_idx_mask(_name, _ele_count, _alloc)                         \
	uint64_t _name##_vla[cdt_idx_vla_mask_count(_ele_count)];                  \
	uint64_t* _name = _name##_vla;                                             \
	cdt_idx_mask_init_temp(&_name, _ele_count, _alloc)

#define define_cond_cdt_idx_mask(_name, _ele_count, _cond, _alloc)             \
	uint64_t _name##_vla[_cond ? cdt_idx_vla_mask_count(_ele_count) : 1];      \
	uint64_t* _name = _name##_vla;                                             \
	if (_cond) {                                                               \
		cdt_idx_mask_init_temp(&_name, _ele_count, _alloc);                    \
	}

#define define_build_order_heap_by_range(_name, _idx, _count, _ele_count,      \
		_udata, _cmp_fn, _success, _alloc)                                     \
	order_heap _name;                                                          \
	uint8_t _name##_vlatemp[1 +                                                \
			cdt_vla_sz(order_index_calc_size(_ele_count, _ele_count))];        \
	DEFER_ATTR(cdt_idx_defer_renull_free_fn)                                   \
	cdt_idx_defer_t _d##_name = { .ordidx = &_name._ };                        \
	bool _name##_##_success =                                                  \
			order_heap_init_build_by_range_temp(&_name, _name##_vlatemp,       \
					&_d##_name, _idx, _count, _ele_count, _cmp_fn, _udata)
...
void* rollback_alloc_copy(rollback_alloc* alloc_buf, const void* buf,
		uint32_t buf_sz);

// msgpacked_index
void msgpacked_index_set(msgpacked_index* idxs, uint32_t index, uint32_t value);
void msgpacked_index_swap(msgpacked_index* idxs, uint32_t index0,
		uint32_t index1);
...
void offset_index_alloc_temp(offset_index* offidx, uint8_t* mem_temp,
		cdt_idx_defer_t* d);
void offset_index_ensure_from_ext_mp(offset_index* offidx, uint32_t ele_count,
...
void order_index_insert(order_index* ordidx, uint32_t idx, uint32_t max_idx,
		uint32_t value);
...
void list_order_index_sort(order_index* ordidx, const offset_index* full_offidx,
...
void map_order_index_sort(order_index* ordidx, const offset_index* offsets,
...
static inline void
_defer_rollback_alloc_internal(rollback_alloc* const* p)
{
	rollback_alloc* alloc = *p;
