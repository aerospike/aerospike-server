...
void as_sindex_put_rd(struct as_sindex_s* si, struct as_storage_rd_s* rd, struct as_index_ref_s* r_ref);

// Modify sindexes from writes/deletes.
uint32_t as_sindex_populate_sbin_si(struct as_sindex_s* si, const as_bin* b, as_sindex_bin* sbins, as_sindex_op op);
...
// Lookup.
struct as_sindex_s* as_sindex_lookup_by_defn(const struct as_namespace_s* ns, uint16_t set_id, const char* bin_name, as_particle_type ktype, as_sindex_type itype, const uint8_t* exp_buf, uint32_t exp_buf_sz, const uint8_t* ctx_buf, uint32_t ctx_buf_sz);
struct as_sindex_s* as_sindex_lookup_by_iname(const struct as_namespace_s* ns, const char* iname);
