...
pre_check(aggr_state* astate)
{
	...
	if (call && call->aggr_hooks && call->aggr_hooks->pre_check) {
		return call->aggr_hooks->pre_check(astate->udata,
				as_rec_source(astate->urec));
	}
...
aopen(aggr_state* astate, const cf_digest* digest)
{
	...
	cf_assert(astate->rsv != NULL, AS_AGGR,
			"reservation not done for partition %d", as_partition_getid(digest));
...
const as_stream_hooks istream_hooks = {
	.destroy = NULL, .read = istream_read, .write = NULL
};
// **************************************************************************************************
...
const as_stream_hooks ostream_hooks = {
	.destroy = NULL, .read = NULL, .write = ostream_write
};
// **************************************************************************************************
...
// **************************************************************************************************
static int
as_aggr_aerospike_log(const as_aerospike* a, const char* file, const int line,
		const int lvl, const char* msg)
{
	cf_log_write(AS_AGGR, lvl, file, line, "%s", (char*)msg);
	return 0;
}

static const as_aerospike_hooks as_aggr_aerospike_hooks = { .rec_update = NULL,
	.rec_remove = NULL,
	.rec_exists = NULL,
		...
		as_aggr_call* ag_call, cf_ll* ap_recl, void* udata, as_result* ap_res)
{
	...
	aggr_state astate = { .iter = cf_ll_getIterator(ap_recl, true /*forward*/),
		.urec = urec,
		.keys_arr = NULL,
		.keys_arr_offset = 0,
		.call = ag_call,
	...
	if (! astate.iter) {
		cf_warning(AS_AGGR,
				"Could not set up iterator .. possibly out of memory .. Aborting Query !!");
		as_rec_destroy(urec);
		return AS_AGGR_ERR;
	}
	...
	int ret = as_module_apply_stream(&mod_lua, &ctx, ag_call->def.filename,
			ag_call->def.function, &istream, ag_call->def.arglist, &ostream,
			ap_res);
