...
eval_hwm_breached(as_namespace* ns)
{
	...
	if (at != reasons) {
		...
		cf_warning(AS_NSUP,
				"{%s} breached eviction limit (%s), sys-memory pct:%u, indexes-memory sz:%lu (%lu + %lu + %lu)%s%s%s, data used-pct:%u",
				ns->name, reasons, sys_memory_pct, ixs_sz, index_mem_sz,
				set_index_sz, sindex_mem_sz, ixs_mem_tag, index_dev_tag,
				sindex_dev_tag, data_used_pct);
	...
	cf_debug(AS_NSUP,
			"{%s} no eviction limit breached, sys-memory pct:%u, indexes-memory sz:%lu (%lu + %lu + %lu)%s%s%s, data used-pct:%u",
