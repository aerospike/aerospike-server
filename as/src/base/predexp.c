...
typedef enum {
	PREDEXP_FALSE = AS_EXP_FALSE,		// matching nodes only
	PREDEXP_TRUE = AS_EXP_TRUE,		// matching nodes only
	PREDEXP_UNKNOWN = AS_EXP_UNK,	// matching nodes only
...
typedef void (*predexp_eval_dtor_fn)(predexp_eval_t* bp); // destructor

typedef predexp_retval_t (*predexp_eval_eval_fn)(predexp_eval_t* bp,
		predexp_args_t* argsp, wrapped_as_bin_t* wbinp);
...
typedef struct as_predexp_eval_old_s {
	uint8_t version;
	predexp_eval_t* root;
} as_predexp_eval_old;
...
static void predexp_eval_base_init(predexp_eval_t* bp, predexp_eval_dtor_fn dtor_fn, predexp_eval_eval_fn eval_fn, uint8_t flags, uint8_t type);
static void destroy_list(predexp_eval_t* bp);
static bool build(predexp_eval_t** stackpp, uint16_t tag, uint32_t len, uint8_t* pp);

static void destroy_and(predexp_eval_t* bp);
static predexp_retval_t eval_and(predexp_eval_t* bp, predexp_args_t* argsp, wrapped_as_bin_t* wbinp);
static bool build_and(predexp_eval_t** stackpp, uint32_t len, uint8_t* pp);

static void destroy_or(predexp_eval_t* bp);
static predexp_retval_t eval_or(predexp_eval_t* bp, predexp_args_t* argsp, wrapped_as_bin_t* wbinp);
static bool build_or(predexp_eval_t** stackpp, uint32_t len, uint8_t* pp);

static void destroy_not(predexp_eval_t* bp);
static predexp_retval_t eval_not(predexp_eval_t* bp, predexp_args_t* argsp, wrapped_as_bin_t* wbinp);
static bool build_not(predexp_eval_t** stackpp, uint32_t len, uint8_t* pp);

static void destroy_compare(predexp_eval_t* bp);
static predexp_retval_t eval_compare(predexp_eval_t* bp, predexp_args_t* argsp, wrapped_as_bin_t* wbinp);
static bool build_compare(predexp_eval_t** stackpp, int32_t len, uint8_t* pp, uint16_t tag);

static void destroy_value(predexp_eval_t* bp);
static predexp_retval_t eval_value(predexp_eval_t* bp, predexp_args_t* argsp, wrapped_as_bin_t* wbinp);
static bool build_value(predexp_eval_t** stackpp, uint32_t len, uint8_t* pp, uint16_t tag);

static void destroy_bin(predexp_eval_t* bp);
static predexp_retval_t eval_bin(predexp_eval_t* bp, predexp_args_t* argsp, wrapped_as_bin_t* wbinp);
static bool build_bin(predexp_eval_t** stackpp, uint32_t len, uint8_t* pp, uint16_t tag);

static void destroy_var(predexp_eval_t* bp);
static predexp_retval_t eval_var(predexp_eval_t* bp, predexp_args_t* argsp, wrapped_as_bin_t* wbinp);
static bool build_var(predexp_eval_t** stackpp, uint32_t len, uint8_t* pp, uint16_t tag);

static void destroy_rec_device_size(predexp_eval_t* bp);
static predexp_retval_t eval_rec_device_size(predexp_eval_t* bp, predexp_args_t* argsp, wrapped_as_bin_t* wbinp);
static bool build_rec_device_size(predexp_eval_t** stackpp, uint32_t len, uint8_t* pp);

static void destroy_rec_last_update(predexp_eval_t* bp);
static predexp_retval_t eval_rec_last_update(predexp_eval_t* bp, predexp_args_t* argsp, wrapped_as_bin_t* wbinp);
static bool build_rec_last_update(predexp_eval_t** stackpp, uint32_t len, uint8_t* pp);

static void destroy_rec_void_time(predexp_eval_t* bp);
static predexp_retval_t eval_rec_void_time(predexp_eval_t* bp, predexp_args_t* argsp, wrapped_as_bin_t* wbinp);
static bool build_rec_void_time(predexp_eval_t** stackpp, uint32_t len, uint8_t* pp);

static void destroy_rec_digest_modulo(predexp_eval_t* bp);
static predexp_retval_t eval_rec_digest_modulo(predexp_eval_t* bp, predexp_args_t* argsp, wrapped_as_bin_t* wbinp);
static bool build_rec_digest_modulo(predexp_eval_t** stackpp, uint32_t len, uint8_t* pp);

static void destroy_iter(predexp_eval_t* bp);
static predexp_retval_t eval_list_iter(predexp_eval_t* bp, predexp_args_t* argsp, wrapped_as_bin_t* wbinp);
static predexp_retval_t eval_map_iter(predexp_eval_t* bp, predexp_args_t* argsp, wrapped_as_bin_t* wbinp);
static bool build_iter(predexp_eval_t** stackpp, uint32_t len, uint8_t* pp, uint16_t tag);
...
as_exp*
predexp_build_old(const as_msg_field* pfp)
{
	predexp_eval_t* stackp = NULL;

	cf_debug(AS_EXP, "%p: predexp_build starting", &stackp);

	uint8_t* pp = (uint8_t*)pfp->data;
	...
	while (pp + 6 <= endp) {
		...
		if (pp + len > endp) {
			cf_warning(AS_EXP, "malformed predexp field");
			goto Failed;
		}
	...
	if (stackp == NULL) {
		cf_warning(AS_EXP, "no top level predexp");
		goto Failed;
	}

	if (stackp->next != NULL) {
		cf_warning(AS_EXP, "multiple top-level predexp");
		goto Failed;
	}

	// The top node needs to be a matching node, not a value node.
	if ((stackp->flags & PREDEXP_VALUE_NODE) != 0) {
		cf_warning(AS_EXP, "top-level predexp is value node");
		goto Failed;
	}

	cf_debug(AS_EXP, "%p: predexp_build finished", &stackp);

	as_predexp_eval_old* ret = cf_malloc(sizeof(as_predexp_eval_old));
	ret->version = 1;
	ret->root = stackp;
...
Failed:
	cf_debug(AS_EXP, "%p: predexp_build failed", &stackp);
	destroy_list(stackp);
	return NULL;
}
		...
		const as_exp_ctx* ctx)
{
	...
	case PREDEXP_UNKNOWN:
		return (as_exp_trilean)result;
	default:
		cf_crash(AS_EXP, "predexp_matches_metadata returned other than true/false/unknown");
		return (as_exp_trilean)PREDEXP_UNKNOWN;
	}
}

bool
predexp_matches_record_old(const as_exp* predexp,
		const as_exp_ctx* ctx)
{
	predexp_eval_t* bp = ((as_predexp_eval_old*)predexp)->root;
	...
	case PREDEXP_FALSE:
		return false;
	default:
		cf_crash(AS_EXP, "predexp_matches_record returned other than true/false with record data present");
		return false;
	}
}

void
predexp_destroy_old(as_exp* predexp)
{
	predexp_eval_t* bp = ((as_predexp_eval_old*)predexp)->root;
...
static void
predexp_eval_base_init(predexp_eval_t* bp, predexp_eval_dtor_fn dtor_fn,
		predexp_eval_eval_fn eval_fn, uint8_t flags, uint8_t type)
{
	bp->next = NULL;
	bp->dtor_fn = dtor_fn;
	bp->eval_fn = eval_fn;
...
build(predexp_eval_t** stackpp, uint16_t tag, uint32_t len, uint8_t* pp)
{
	...
	default:
		cf_warning(AS_EXP, "unexpected predexp tag: %d", tag);
		return false;
	}
}
...
{
	predexp_eval_and_t* dp = (predexp_eval_and_t*)bp;
	predexp_retval_t retval = PREDEXP_TRUE; // start optimistically
	...
	for (predexp_eval_t* cp = dp->child; cp != NULL; cp = cp->next) {
		...
		case PREDEXP_NOVALUE:
			...
			cf_crash(AS_EXP, "eval_and child was value node");
		}
	}
...
build_and(predexp_eval_t** stackpp, uint32_t len, uint8_t* pp)
{
	if (len != sizeof(uint16_t)) {
		cf_warning(AS_EXP, "predexp_and: unexpected size %d", len);
		return false;
	}
	...
	pp += sizeof(uint16_t);
	predexp_eval_base_init((predexp_eval_t*)dp, destroy_and, eval_and, 0,
			AS_PARTICLE_TYPE_NULL); // start optimistically
	dp->child = NULL;

	for (uint16_t ndx = 0; ndx < nterms; ndx++) {
		// If there is not an available child expr cleanup and fail.
		if (*stackpp == NULL) {
			cf_warning(AS_EXP, "predexp_and: missing child %d", ndx);
		...
		*stackpp = child->next;		// Stack points around the child.
		...
		if ((dp->child->flags & PREDEXP_VALUE_NODE) != 0) {
			cf_warning(AS_EXP, "predexp_and: child %d is value node", ndx);
			(*dp->base.dtor_fn)((predexp_eval_t*)dp);
			return false;
		}
	}
	...
	*stackpp = (predexp_eval_t*)dp;	// We're the new top
	...
	cf_debug(AS_EXP, "%p: predexp_and(%d)", stackpp, nterms);
...
{
	predexp_eval_or_t* dp = (predexp_eval_or_t*)bp;
	predexp_retval_t retval = PREDEXP_FALSE; // start pessimistically
	...
	for (predexp_eval_t* cp = dp->child; cp != NULL; cp = cp->next) {
		...
		case PREDEXP_NOVALUE:
			// Child can't be value node; shouldn't ever happen.
			cf_crash(AS_EXP, "eval_or child was value node");
		}
	}
...
{
	if (len != sizeof(uint16_t)) {
		cf_warning(AS_EXP, "predexp_or: unexpected size %d", len);
		return false;
	}
	...
	pp += sizeof(uint16_t);
	predexp_eval_base_init((predexp_eval_t*)dp, destroy_or, eval_or, 0,
	...
	for (uint16_t ndx = 0; ndx < nterms; ndx++) {
		// If there is not an available child expr cleanup and fail.
		if (*stackpp == NULL) {
			cf_warning(AS_EXP, "predexp_or: missing child %d", ndx);
			(*dp->base.dtor_fn)((predexp_eval_t*)dp);
			return false;
		}
		...
		*stackpp = child->next;		// Stack points around the child.
		...
		// Make sure the child is not a value node.
		if ((dp->child->flags & PREDEXP_VALUE_NODE) != 0) {
			cf_warning(AS_EXP, "predexp_or: child %d is value node", ndx);
			(*dp->base.dtor_fn)((predexp_eval_t*)dp);
			return false;
		}
	}
	...
	*stackpp = (predexp_eval_t*)dp;	// We're the new top
	...
	cf_debug(AS_EXP, "%p: predexp_or(%d)", stackpp, nterms);
...
eval_not(predexp_eval_t* bp, predexp_args_t* argsp, wrapped_as_bin_t* wbinp)
{
	...
	case PREDEXP_NOVALUE:
		// Child can't be value node; shouldn't ever happen.
		cf_crash(AS_EXP, "eval_not child was value node");
	}
...
{
	if (len != 0) {
		cf_warning(AS_EXP, "predexp_not: unexpected size %d", len);
		return false;
	}
	...
	predexp_eval_base_init((predexp_eval_t*)dp, destroy_not, eval_not, 0,
			AS_PARTICLE_TYPE_NULL);
	dp->child = NULL;

	// If there is not an available child expr cleanup and fail.
	if (*stackpp == NULL) {
		cf_warning(AS_EXP, "predexp_not: missing child");
		(*dp->base.dtor_fn)((predexp_eval_t*)dp);
		return false;
	}
	...
	*stackpp = child->next;		// Stack points around the child.
	...
	// Make sure the child is not a value node.
	if ((dp->child->flags & PREDEXP_VALUE_NODE) != 0) {
		cf_warning(AS_EXP, "predexp_not: child is value node");
		(*dp->base.dtor_fn)((predexp_eval_t*)dp);
		return false;
	}
	...
	*stackpp = (predexp_eval_t*)dp;	// We're the new top
	...
	cf_debug(AS_EXP, "%p: predexp_not", stackpp);
...
//==========================================================
// Local helpers - AS_PREDEXP_*_COMPARE
//
...
destroy_compare(predexp_eval_t* bp)
{
	...
	if (dp->type == AS_PARTICLE_TYPE_GEOJSON &&
			dp->state.geojson.region != NULL) {
...
static predexp_retval_t
eval_compare(predexp_eval_t* bp, predexp_args_t* argsp, wrapped_as_bin_t* wbinp)
{
	predexp_eval_compare_t* dp = (predexp_eval_compare_t *)bp;
	...
	case AS_PARTICLE_TYPE_INTEGER: {
		...
		case AS_PREDEXP_INTEGER_LESSEQ:
			...
			goto Cleanup;
		default:
			cf_crash(AS_EXP, "eval_compare integer unknown tag %d",
					dp->tag);
		}
	}
	// no break
	case AS_PARTICLE_TYPE_STRING: {
		...
		case AS_PREDEXP_STRING_UNEQUAL:
			...
			case AS_PREDEXP_STRING_UNEQUAL:
				...
				goto Cleanup;
			default:
				cf_crash(AS_EXP, "eval_compare string (eq) unknown tag %d",
						dp->tag);
			}
		case AS_PREDEXP_STRING_REGEX: {
		...
		default:
			cf_crash(AS_EXP, "eval_compare string unknown tag %d", dp->tag);
		}
	}
	// no break
	case AS_PARTICLE_TYPE_GEOJSON: {
		...
		default:
			cf_crash(AS_EXP, "eval_compare geojson unknown tag %d",
					dp->tag);
		}
	}
	// no break
	default:
		cf_crash(AS_EXP, "eval_compare unknown type %d", dp->type);
	}

Cleanup:
	if (lwbin.must_free) {
		cf_crash(AS_EXP, "eval_compare need bin cleanup, didn't before");
	}
	if (rwbin.must_free) {
		cf_crash(AS_EXP, "eval_compare need bin cleanup, didn't before");
	}
...
static bool
build_compare(predexp_eval_t** stackpp, int32_t len, uint8_t* pp, uint16_t tag)
{
	predexp_eval_compare_t* dp =
			(predexp_eval_compare_t*)cf_malloc(sizeof(predexp_eval_compare_t));

	predexp_eval_base_init((predexp_eval_t*)dp, destroy_compare, eval_compare,
	...
	case AS_PREDEXP_GEOJSON_CONTAINS:
		...
		break;
	default:
		cf_crash(AS_EXP, "build_compare called with bogus tag: %d", tag);
		break;
	}
	...
	if (tag == AS_PREDEXP_STRING_REGEX) {
		...
		if (pp + sizeof(uint32_t) > endp) {
			cf_warning(AS_EXP, "build_compare: regex opts past end");
			(*dp->base.dtor_fn)((predexp_eval_t*)dp);
			return false;
		}
	...
	// No arguments.
	if (pp != endp) {
		cf_warning(AS_EXP, "build_compare: msg unaligned");
		(*dp->base.dtor_fn)((predexp_eval_t*)dp);
		return false;
	}
	...
	if (*stackpp == NULL) {
		cf_warning(AS_EXP, "predexp_compare: missing right child");
		(*dp->base.dtor_fn)((predexp_eval_t*)dp);
		return false;
	}
	...
	*stackpp = dp->rchild->next;
	...
	if ((dp->rchild->flags & PREDEXP_VALUE_NODE) == 0) {
		cf_warning(AS_EXP, "predexp compare: right child is not value node");
		(*dp->base.dtor_fn)((predexp_eval_t*)dp);
		return false;
	}

	if (dp->rchild->type != dp->type) {
		cf_warning(AS_EXP, "predexp compare: right child is wrong type");
		(*dp->base.dtor_fn)((predexp_eval_t*)dp);
		return false;
	}
	...
	if (*stackpp == NULL) {
		cf_warning(AS_EXP, "predexp_compare: missing left child");
		(*dp->base.dtor_fn)((predexp_eval_t*)dp);
		return false;
	}
	...
	*stackpp = dp->lchild->next;
	...
	if ((dp->lchild->flags & PREDEXP_VALUE_NODE) == 0) {
		cf_warning(AS_EXP, "predexp compare: left child is not value node");
		(*dp->base.dtor_fn)((predexp_eval_t*)dp);
		return false;
	}

	if (dp->lchild->type != dp->type) {
		cf_warning(AS_EXP, "predexp compare: left child is wrong type");
		(*dp->base.dtor_fn)((predexp_eval_t*)dp);
		return false;
	}
	...
	case AS_PREDEXP_GEOJSON_CONTAINS:
		...
		if ((dp->rchild->flags & PREDEXP_IMMEDIATE_NODE) == 0) {
			cf_warning(AS_EXP, "predexp compare: within arg not immediate GeoJSON");
			(*dp->base.dtor_fn)((predexp_eval_t*)dp);
			return false;
		}
		...
		if ((*dp->rchild->eval_fn)(dp->rchild, argsp, &rwbin) ==
				PREDEXP_NOVALUE) {
			cf_warning(AS_EXP, "predexp compare: within arg had unknown value");
			(*dp->base.dtor_fn)((predexp_eval_t*)dp);
			return false;
		}
		...
		if (! geo_parse(NULL, ptr, sz, &dp->state.geojson.cellid,
				&dp->state.geojson.region)) {
			cf_warning(AS_EXP, "predexp compare: failed to parse GeoJSON");
			(*dp->base.dtor_fn)((predexp_eval_t*)dp);

			if (rwbin.must_free) {
				cf_crash(AS_EXP, "predexp compare now needs bin destructor");
			}
	...
	case AS_PREDEXP_STRING_REGEX:
		// The right child needs to be an immediate value.
		if ((dp->rchild->flags & PREDEXP_IMMEDIATE_NODE) == 0) {
			cf_warning(AS_EXP, "predexp compare: regex arg not immediate string");
			(*dp->base.dtor_fn)((predexp_eval_t*)dp);
			return false;
		}
		...
		if ((*dp->rchild->eval_fn)(dp->rchild, argsp2, &rwbin2) ==
				PREDEXP_NOVALUE) {
			cf_warning(AS_EXP, "predexp compare: regex arg had unknown value");
			(*dp->base.dtor_fn)((predexp_eval_t*)dp);
			return false;
		}
		...
		if (rv != 0) {
			...
			cf_warning(AS_EXP, "predexp compare: regex compile failed: %s",
					errbuf);
			(*dp->base.dtor_fn)((predexp_eval_t*)dp);
	...
	*stackpp = (predexp_eval_t*)dp; // We're the new top
	...
	switch (tag) {
	case AS_PREDEXP_INTEGER_EQUAL:
		cf_debug(AS_EXP, "%p: predexp_integer_equal", stackpp);
		break;
	case AS_PREDEXP_INTEGER_UNEQUAL:
		cf_debug(AS_EXP, "%p: predexp_integer_unequal", stackpp);
		break;
	case AS_PREDEXP_INTEGER_GREATER:
		cf_debug(AS_EXP, "%p: predexp_integer_greater", stackpp);
		break;
	case AS_PREDEXP_INTEGER_GREATEREQ:
		cf_debug(AS_EXP, "%p: predexp_integer_greatereq", stackpp);
		break;
	case AS_PREDEXP_INTEGER_LESS:
		cf_debug(AS_EXP, "%p: predexp_integer_less", stackpp);
		break;
	case AS_PREDEXP_INTEGER_LESSEQ:
		cf_debug(AS_EXP, "%p: predexp_integer_lesseq", stackpp);
		break;
	case AS_PREDEXP_STRING_EQUAL:
		cf_debug(AS_EXP, "%p: predexp_string_equal", stackpp);
		break;
	case AS_PREDEXP_STRING_UNEQUAL:
		cf_debug(AS_EXP, "%p: predexp_string_unequal", stackpp);
		break;
	case AS_PREDEXP_STRING_REGEX:
		cf_debug(AS_EXP, "%p: predexp_string_regex(%d)", stackpp,
				regex_opts);
		break;
	case AS_PREDEXP_GEOJSON_WITHIN:
		cf_debug(AS_EXP, "%p: predexp_geojson_within", stackpp);
		break;
	case AS_PREDEXP_GEOJSON_CONTAINS:
		cf_debug(AS_EXP, "%p: predexp_geojson_contains", stackpp);
		break;
	default:
		cf_crash(AS_EXP, "build_compare called with bogus tag: %d", tag);
		break;
...
//==========================================================
// Local helpers - AS_PREDEXP_*_VALUE
//
...
{
	cf_assert(wbinp != NULL, AS_EXP, "eval_value called outside value context");
...
{
	predexp_eval_value_t* dp =
			(predexp_eval_value_t*)cf_malloc(sizeof(predexp_eval_value_t));
	uint8_t type;
	...
	case AS_PREDEXP_INTEGER_VALUE:
		...
		break;
	case AS_PREDEXP_STRING_VALUE:
		type = AS_PARTICLE_TYPE_STRING;
		break;
	case AS_PREDEXP_GEOJSON_VALUE:
		type = AS_PARTICLE_TYPE_GEOJSON;
		break;
	default:
		cf_crash(AS_EXP, "build_value called with bogus tag: %d", tag);
		return false;
	}

	predexp_eval_base_init((predexp_eval_t*)dp, destroy_value, eval_value,
			PREDEXP_VALUE_NODE | PREDEXP_IMMEDIATE_NODE, type);
	...
	if (pp != endp) {
		cf_warning(AS_EXP, "predexp value: msg unaligned");
		goto Failed;
	}
	...
	int result = particle_vtable[type]->from_wire_fn(type, valptr, vallen,
			&dp->bin.particle);
	...
	if (result == 0) {
		...
		cf_warning(AS_EXP, "failed to build predexp value with err %d",
				result);
	...
	*stackpp = (predexp_eval_t*)dp; // we're the new top
	...
	switch (tag) {
	case AS_PREDEXP_INTEGER_VALUE:
		cf_debug(AS_EXP, "%p: predexp_integer_value(%"PRId64")", stackpp,
				(int64_t) dp->bin.particle);
		break;
	case AS_PREDEXP_STRING_VALUE: {
		cf_debug(AS_EXP, "%p: predexp_string_value(\"%.*s\")", stackpp,
	...
	case AS_PREDEXP_GEOJSON_VALUE: {
		...
		cf_debug(AS_EXP, "%p: predexp_geojson_value(%.*s)", stackpp,
...
//==========================================================
// Local helpers - AS_PREDEXP_*_BIN
//
...
{
	if (wbinp == NULL) {
		cf_crash(AS_EXP, "eval_bin called outside value context");
	}
...
{
	predexp_eval_bin_t* dp =
			(predexp_eval_bin_t*)cf_malloc(sizeof(predexp_eval_bin_t));
	...
	case AS_PREDEXP_MAP_BIN:
		...
		break;
	default:
		cf_crash(AS_EXP, "build_bin called with bogus tag: %d", tag);
		break;
	}

	predexp_eval_base_init((predexp_eval_t*)dp, destroy_bin, eval_bin,
			PREDEXP_VALUE_NODE, dp->type);

	uint8_t* endp = pp + len;

	if (len >= sizeof(dp->bname)) {
		cf_warning(AS_EXP, "build_bin: binname too long");
		goto Failed;
	}
	...
	if (pp != endp) {
		cf_warning(AS_EXP, "build_bin: msg unaligned");
		goto Failed;
	}
	...
	*stackpp = (predexp_eval_t*)dp; // We're the new top
	...
	switch (tag) {
	case AS_PREDEXP_INTEGER_BIN:
		cf_debug(AS_EXP, "%p: predexp_integer_bin(\"%s\")", stackpp,
				dp->bname);
		break;
	case AS_PREDEXP_STRING_BIN:
		cf_debug(AS_EXP, "%p: predexp_string_bin(\"%s\")", stackpp,
				dp->bname);
		break;
	case AS_PREDEXP_GEOJSON_BIN:
		cf_debug(AS_EXP, "%p: predexp_geojson_bin(\"%s\")", stackpp,
				 dp->bname);
		break;
	case AS_PREDEXP_LIST_BIN:
		cf_debug(AS_EXP, "%p: predexp_list_bin(\"%s\")", stackpp,
				 dp->bname);
		break;
	case AS_PREDEXP_MAP_BIN:
		cf_debug(AS_EXP, "%p: predexp_map_bin(\"%s\")", stackpp,
				 dp->bname);
		break;
	default:
		cf_crash(AS_EXP, "build_bin called with bogus tag: %d", tag);
...
//==========================================================
// Local helpers - AS_PREDEXP_*_VAR
//
...
{
	if (wbinp == NULL) {
		cf_crash(AS_EXP, "eval_var called outside value context");
	}
...
{
	predexp_eval_var_t* dp =
			(predexp_eval_var_t*)cf_malloc(sizeof(predexp_eval_var_t));
	...
	case AS_PREDEXP_GEOJSON_VAR:
		...
		break;
	default:
		cf_crash(AS_EXP, "build_var called with bogus tag: %d", tag);
		break;
	}

	predexp_eval_base_init((predexp_eval_t*)dp, destroy_var, eval_var,
			PREDEXP_VALUE_NODE, dp->type);

	uint8_t* endp = pp + len;

	if (len >= sizeof(dp->vname)) {
		cf_warning(AS_EXP, "build_var: varname too long");
		goto Failed;
	}
	...
	if (pp != endp) {
		cf_warning(AS_EXP, "build_var: msg unaligned");
		goto Failed;
	}
	...
	*stackpp = (predexp_eval_t*)dp; // We're the new top
	...
	switch (tag) {
	case AS_PREDEXP_INTEGER_VAR:
		cf_debug(AS_EXP, "%p: predexp_integer_var(\"%s\")", stackpp,
				 dp->vname);
		break;
	case AS_PREDEXP_STRING_VAR:
		cf_debug(AS_EXP, "%p: predexp_string_var(\"%s\")", stackpp,
				 dp->vname);
		break;
	case AS_PREDEXP_GEOJSON_VAR:
		cf_debug(AS_EXP, "%p: predexp_geojson_var(\"%s\")", stackpp,
				 dp->vname);
		break;
	default:
		cf_crash(AS_EXP, "build_var called with bogus tag: %d", tag);
...
//==========================================================
// Local helpers - AS_PREDEXP_REC_DEVICE_SIZE
//
...
static predexp_retval_t
eval_rec_device_size(predexp_eval_t* bp, predexp_args_t* argsp,
...
build_rec_device_size(predexp_eval_t** stackpp, uint32_t len, uint8_t* pp)
{
	...
	predexp_eval_base_init((predexp_eval_t*)dp, destroy_rec_device_size,
			eval_rec_device_size, PREDEXP_VALUE_NODE, AS_PARTICLE_TYPE_INTEGER);

	uint8_t* endp = pp + len;

	if (pp != endp) {
		cf_warning(AS_EXP, "build_rec_device_size: msg unaligned");
		(*dp->base.dtor_fn)((predexp_eval_t*)dp);
		return false;
	}
	...
	*stackpp = (predexp_eval_t*)dp;	// We're the new top
	...
	cf_debug(AS_EXP, "%p: predexp_rec_device_size()", stackpp);
...
//==========================================================
// Local helpers - AS_PREDEXP_REC_LAST_UPDATE
//
...
static predexp_retval_t
eval_rec_last_update(predexp_eval_t* bp, predexp_args_t* argsp,
		wrapped_as_bin_t* wbinp)
{
	cf_assert(wbinp != NULL, AS_EXP, "eval_rec_last_update called outside value context");
...
build_rec_last_update(predexp_eval_t** stackpp, uint32_t len, uint8_t* pp)
{
	...
	predexp_eval_base_init((predexp_eval_t*)dp, destroy_rec_last_update,
			eval_rec_last_update, PREDEXP_VALUE_NODE, AS_PARTICLE_TYPE_INTEGER);

	uint8_t* endp = pp + len;

	if (pp != endp) {
		cf_warning(AS_EXP, "build_rec_last_update: msg unaligned");
		(*dp->base.dtor_fn)((predexp_eval_t*)dp);
		return false;
	}
	...
	*stackpp = (predexp_eval_t*)dp;	// We're the new top
	...
	cf_debug(AS_EXP, "%p: predexp_rec_last_update()", stackpp);
...
//==========================================================
// Local helpers - AS_PREDEXP_REC_VOID_TIME
//
...
static predexp_retval_t
eval_rec_void_time(predexp_eval_t* bp, predexp_args_t* argsp,
		wrapped_as_bin_t* wbinp)
{
	cf_assert(wbinp != NULL, AS_EXP, "eval_rec_void_time called outside value context");
...
build_rec_void_time(predexp_eval_t** stackpp, uint32_t len, uint8_t* pp)
{
	...
	predexp_eval_base_init((predexp_eval_t*)dp, destroy_rec_void_time,
			eval_rec_void_time, PREDEXP_VALUE_NODE, AS_PARTICLE_TYPE_INTEGER);

	uint8_t* endp = pp + len;

	if (pp != endp) {
		cf_warning(AS_EXP, "build_rec_void_time: msg unaligned");
		(*dp->base.dtor_fn)((predexp_eval_t*)dp);
		return false;
	}
	...
	*stackpp = (predexp_eval_t*)dp;	// We're the new top
	...
	cf_debug(AS_EXP, "%p: predexp_rec_void_time()", stackpp);
...
//==========================================================
// Local helpers - AS_PREDEXP_REC_DIGEST_MODULO
//
...
static predexp_retval_t
eval_rec_digest_modulo(predexp_eval_t* bp, predexp_args_t* argsp,
		wrapped_as_bin_t* wbinp)
{
	cf_assert(wbinp != NULL, AS_EXP, "eval_rec_digest_modulo called outside value context");
...
build_rec_digest_modulo(predexp_eval_t** stackpp, uint32_t len, uint8_t* pp)
{
	...
	predexp_eval_base_init((predexp_eval_t*)dp, destroy_rec_digest_modulo,
			eval_rec_digest_modulo, PREDEXP_VALUE_NODE,
			AS_PARTICLE_TYPE_INTEGER);

	uint8_t* endp = pp + len;

	if (pp + sizeof(int32_t) > endp) {
		cf_warning(AS_EXP, "build_rec_digest_modulo: msg too short");
		goto Failed;
	}
	...
	if (pp != endp) {
		cf_warning(AS_EXP, "build_rec_digest_modulo: msg unaligned");
		goto Failed;
	}

	if (dp->mod == 0) {
		cf_warning(AS_EXP, "build_rec_digest_modulo: zero modulo invalid");
		goto Failed;
	}
	...
	*stackpp = (predexp_eval_t*)dp; // We're the new top
	...
	cf_debug(AS_EXP, "%p: predexp_rec_digest_modulo(%d)", stackpp, dp->mod);
...
//==========================================================
// Local helpers - AS_PREDEXP_*_ITERATE_*
//
...
static predexp_retval_t
eval_list_iter(predexp_eval_t* bp, predexp_args_t* argsp,
		wrapped_as_bin_t* wbinp)
{
	predexp_eval_iter_t* dp = (predexp_eval_iter_t*)bp;
	...
	case AS_PREDEXP_LIST_ITERATE_AND:
		...
		break;
	default:
		cf_crash(AS_EXP, "eval_list_iter called with bogus tag: %d",
				dp->tag);
	}
	...
	if ((*dp->rchild->eval_fn)(dp->rchild, argsp, &lwbin) == PREDEXP_NOVALUE) {
	...
	while (as_arraylist_iterator_has_next(&it)) {
		...
		if (rv != 0) {
			cf_warning(AS_EXP, "eval_list_iter: particle from asval failed");
			continue;
		}
		...
		case AS_PREDEXP_LIST_ITERATE_OR:
			...
			case PREDEXP_NOVALUE:
				// Child can't be value node; shouldn't ever happen.
				cf_crash(AS_EXP, "eval_list_iter child was value node");
			}
			break;
		case AS_PREDEXP_LIST_ITERATE_AND:
			switch ((*dp->lchild->eval_fn)(dp->lchild, argsp, NULL)) {
...
Done:
	...
	if (lwbin.must_free) {
		cf_crash(AS_EXP, "eval_list_iter need bin cleanup, didn't before");
	}
...
static predexp_retval_t
eval_map_iter(predexp_eval_t* bp, predexp_args_t* argsp,
		wrapped_as_bin_t* wbinp)
{
	predexp_eval_iter_t* dp = (predexp_eval_iter_t*)bp;
	predexp_retval_t retval = PREDEXP_UNKNOWN; // init makes compiler happy
	...
	case AS_PREDEXP_MAPVAL_ITERATE_AND:
		...
		break;
	default:
		cf_crash(AS_EXP, "eval_map_iter called with bogus tag: %d",
				dp->tag);
	}
	...
	while (as_hashmap_iterator_has_next(&it)) {
		...
		case AS_PREDEXP_MAPVAL_ITERATE_AND:
			...
			break;
		default:
			cf_crash(AS_EXP, "eval_map_iter called with bogus tag (2): %d",
					dp->tag);
		}
		...
		if (rv != 0) {
			cf_warning(AS_EXP, "eval_map_iter: particle from asval failed");
			continue;
		}
		...
		case AS_PREDEXP_MAPVAL_ITERATE_OR:
			...
			case PREDEXP_NOVALUE:
				// Child can't be value node; shouldn't ever happen.
				cf_crash(AS_EXP, "eval_map_iter child was value node");
			}
			break;
		case AS_PREDEXP_MAPKEY_ITERATE_AND:
		case AS_PREDEXP_MAPVAL_ITERATE_AND:
...
Done:
	...
	if (lwbin.must_free) {
		cf_crash(AS_EXP, "eval_map_iter need bin cleanup, didn't before");
	}
...
{
	predexp_eval_iter_t* dp =
			(predexp_eval_iter_t*)cf_malloc(sizeof(predexp_eval_iter_t));

	switch (tag) {
	case AS_PREDEXP_LIST_ITERATE_OR:
	case AS_PREDEXP_LIST_ITERATE_AND:
		predexp_eval_base_init((predexp_eval_t*)dp, destroy_iter,
				eval_list_iter, 0, AS_PARTICLE_TYPE_NULL);
		dp->type = AS_PARTICLE_TYPE_LIST;
		break;
	case AS_PREDEXP_MAPKEY_ITERATE_OR:
	case AS_PREDEXP_MAPVAL_ITERATE_OR:
	case AS_PREDEXP_MAPKEY_ITERATE_AND:
	case AS_PREDEXP_MAPVAL_ITERATE_AND:
		predexp_eval_base_init((predexp_eval_t*)dp, destroy_iter, eval_map_iter,
				0, AS_PARTICLE_TYPE_NULL);
		dp->type = AS_PARTICLE_TYPE_MAP;
		break;
	default:
		cf_crash(AS_EXP, "build_iter called with bogus tag: %d", tag);
	}
	...
	if (len >= sizeof(dp->vname)) {
		cf_warning(AS_EXP, "build_iter: varname too long");
		goto Failed;
	}
	...
	if (*stackpp == NULL) {
		cf_warning(AS_EXP, "predexp_iterate: missing right child");
		(*dp->base.dtor_fn)((predexp_eval_t*)dp);
		return false;
	}
	...
	*stackpp = dp->rchild->next;
	...
	if ((dp->rchild->flags & PREDEXP_VALUE_NODE) == 0) {
		cf_warning(AS_EXP, "predexp iterate: right child is not value node");
		(*dp->base.dtor_fn)((predexp_eval_t*)dp);
		return false;
	}

	if (dp->rchild->type != dp->type) {
		cf_warning(AS_EXP, "predexp iterate: right child is wrong type");
		(*dp->base.dtor_fn)((predexp_eval_t*)dp);
		return false;
	}
	...
	if (*stackpp == NULL) {
		cf_warning(AS_EXP, "predexp_iterate: missing left child");
		(*dp->base.dtor_fn)((predexp_eval_t*)dp);
		return false;
	}
	...
	*stackpp = dp->lchild->next;
	...
	if ((dp->lchild->flags & PREDEXP_VALUE_NODE) == 1) {
		cf_warning(AS_EXP, "predexp iterate: left child is value node");
		(*dp->base.dtor_fn)((predexp_eval_t*)dp);
		return false;
	}

	if (dp->lchild->type != AS_PARTICLE_TYPE_NULL) {
		cf_warning(AS_EXP, "predexp iterate: left child is wrong type");
		(*dp->base.dtor_fn)((predexp_eval_t*)dp);
		return false;
	}

	if (pp != endp) {
		cf_warning(AS_EXP, "build_iter: msg unaligned");
		goto Failed;
	}
	...
	*stackpp = (predexp_eval_t*)dp; // we're the new top
	...
	switch (tag) {
	case AS_PREDEXP_LIST_ITERATE_OR:
		cf_debug(AS_EXP, "%p: predexp_list_iterate_or()", stackpp);
		break;
	case AS_PREDEXP_LIST_ITERATE_AND:
		cf_debug(AS_EXP, "%p: predexp_list_iterate_and()", stackpp);
		break;
	case AS_PREDEXP_MAPKEY_ITERATE_OR:
		cf_debug(AS_EXP, "%p: predexp_mapkey_iterate_or()", stackpp);
		break;
	case AS_PREDEXP_MAPVAL_ITERATE_OR:
		cf_debug(AS_EXP, "%p: predexp_mapval_iterate_or()", stackpp);
		break;
	case AS_PREDEXP_MAPKEY_ITERATE_AND:
		cf_debug(AS_EXP, "%p: predexp_mapkey_iterate_and()", stackpp);
		break;
	case AS_PREDEXP_MAPVAL_ITERATE_AND:
		cf_debug(AS_EXP, "%p: predexp_mapval_iterate_and()", stackpp);
		break;
	default:
		cf_crash(AS_EXP, "build_iter called with bogus tag: %d", tag);
	}
